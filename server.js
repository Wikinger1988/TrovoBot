...
app.use(
    '/overlay',
    require(path.join(__dirname, 'app', 'routes', 'overlay.js'))
)
...
io.on('connection', (_socket) => {
    _socket.on('??', (incomingData) => {
        vorpal.log(incomingData)
    })
})
...
server.listen(process.env.PORTAL_PORT, () => {
    ...
    if (settings) {
        ...
        if (settings.autoconnect) {
            vorpal.log(
                'Auto-Connect is enabled, Trovobot is now Connecting....'
            )
...
vorpal
    .command(
        'settings',
        'Opens a Browser Window to the Trovobot Settings Page in the Control Panel.'
    )
    .action((args, callback) => {
        this.log(
            '\r\nOpening Control Panel to Settings Page in the Browser now....'
                .brightGreen
        )
